...
class FoolsGold(AggregatorBase):
    ...
    def aggregate(self, updates, **kwargs):
        ...
        n_clients = self.args.total_clients if len(self.checkpoints) == 0 else self.args.num_clients
        
        # weights for updates to re-weight for clients' updates
        wv = np.zeros((n_clients, 1), dtype=np.float32)
        ...
        # 如果是第一轮，只保存前10个客户端的更新
        if len(self.checkpoints) == 0:
            self.checkpoints.append(copy.deepcopy(gradient_updates[:10]))
            # 第一轮只使用前10个客户端的更新进行聚合
            gradient_updates = gradient_updates[:10]
            n_clients = 10
        else:
        ...
        cos_dist = smp.cosine_similarity(
            sumed_updates[:, indicative_mask == 1]) - np.eye(n_clients, dtype=np.float32)

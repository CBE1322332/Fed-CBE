...
class ModelReplacement(MPBase, DPBase, Client):
    ...
    def __init__(self, args, worker_id, train_dataset, test_dataset):
        ...
        self.default_attack_params = {
            ...
            "poisoning_ratio": 0.32, "target_label": 6, "source_label": 3, "attack_strategy": "continuous", "single_epoch": 0, "poison_frequency": 5, "attack_start_epoch": None}
        self.update_and_set_attr()

        self.define_synthesizer()
        self.poison_epochs = self.generate_poison_epochs(
            self.attack_strategy, self.args.epochs, self.single_epoch, self.poison_frequency, self.attack_start_epoch)
        self.train_loader = self.get_dataloader(
            train_dataset, train_flag=True, poison_epochs=self.poison_epochs)
        self.algorithm = "FedOpt"
        
        # 初始化global_weights_vec
        self.global_weights_vec = model2vec(self.model)
